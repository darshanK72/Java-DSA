...
public class j04GetMazePathsDownRightWithObstacles {
    ...
    public static void main(String args[]) {
        ...
        // Approach 1: Print paths directly using recursion
        printMazePaths(0, 0, 2, 2, "", maze);

        // Approach 2: Collect paths recursively into a list
        System.out.println(getMazePaths(0, 0, 2, 2, maze));

        // Approach 3: Store paths in an external list using recursion
        ArrayList<String> set = new ArrayList<>();
        generateMazePaths(0, 0, 2, 2, "", maze, set);
    ...
    /**
     * Approach 1: Print all paths using recursion (without returning an ArrayList)
     * 
     * Intuition:
    ...
    /**
     * Approach 2: Collect all paths using recursion and return an ArrayList<String>
     * 
     * Intuition:
    ...
    public static ArrayList<String> getMazePaths(int sr, int sc, int dr, int dc, boolean[][] maze) {
        ...
        // Move right
        if (sc < dc) {
            for (String path : getMazePaths(sr, sc + 1, dr, dc, maze)) {
                output.add("R" + path);
            }
        }
        ...
        // Move down
        if (sr < dr) {
            for (String path : getMazePaths(sr + 1, sc, dr, dc, maze)) {
                output.add("D" + path);
            }
        }
     ...
     */
    public static void generateMazePaths(int sr, int sc, int dr, int dc, String path, boolean[][] maze,
        ...
        generateMazePaths(sr, sc + 1, dr, dc, path + "R", maze, set);
        generateMazePaths(sr + 1, sc, dr, dc, path + "D", maze, set);
    }
}

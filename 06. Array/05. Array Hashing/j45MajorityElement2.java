...
public class j45MajorityElement2{
    ...
    public static ArrayList<Integer> majorityElement2Efficient(int[] arr){
        ...
        int cb = 0;
        for(int i = 0; i < arr.length; i++){
            if(majorityA == arr[i]) ca++;
            if(majorityB == arr[i]) cb++;
        }
        if(ca > arr.length / 3) output.add(majorityA);
        if(cb > arr.length / 3) output.add(majorityB);
        return output;
    }
}